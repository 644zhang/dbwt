...
for ii=6:6
    ...
    %m1= imread(strcat('F:\segdata\labeldatazxy\return1\train',int2str(ii),'.tiff')); 
    m1= imread(strcat('F:\segdata\db3\result1\db2_31_',int2str(ii),'.tiff')); 
  %figure;
  %imshow(m1);
...
smallobj{ii}=center{ii}(smalllocate,:);
% h=figure()
% imshow(bw4);hold on;
...
% figure,imshow(segimg,[])
%save('normal2.mat','bound_small','mask_small','center_small','smallobj','center','bound','rmset','mask');
